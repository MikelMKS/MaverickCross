...
<div class="modal-header">
    <h4 class="modal-title col-12 text-center titulomodal">FIN SEMANA</h5>
</div>
{{--  --}} 
...
<div class="modal-footer">
    @if($visto[0]->visto == 0)
        <button type="button" class="btn btn-success btn-sm" onclick="enteradoFinSemana('{{$visto[0]->id}}')">ENTERADO</button>
    @endif
    <button type="button" class="btn btn-secondary btn-sm" onclick="$('#modalverNotificacion').modal('hide');">CERRAR</button>
</div>

<script>
    function enteradoFinSemana(id){
        $.ajax({
            data: { 'id':id, _token: "{{ csrf_token() }}" },
            type : "GET",
            url : "{{route('enteradoFinSemana')}}",
            beforeSend : function () {
                swalLoading();
            },
